...
import static com.easyder.wrapper.utils.DateUtils.YEAR_MONTH_DAY_HOUR_MINUTE_SECOND;
import static com.easyder.wrapper.utils.DateUtils.YEAR_MONTH_DAY_HOUR_MINUTE_SECOND2;
import static com.easyder.wrapper.utils.DateUtils.YEAR_MONTH_DAY_HOUR_MINUTE_SECOND4;
...
public class CommonTools {
    ...
    public static String getFNBFormantTime(double time){
        String timeString = new BigDecimal(time).toString();
        String format = getFormatTime(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND2,timeString);
        return format;
    }
    public static String getFNBFormantTime1(double time){
        String timeString = new BigDecimal(time).toString();
        String format = getFormatTime(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND,timeString);
        return format;
    }

    public static String getFNBFormantTime4(long time){
        if(time <= 0) {
            time = System.currentTimeMillis();
        }
        String timeString = new BigDecimal(time).toString();
        String format = getFormatTime(YEAR_MONTH_DAY_HOUR_MINUTE_SECOND4, timeString);
        return format;
    }
    ...
    /**
     * SimpleDateFormat 线程不安全，如果并发请求，会出现各种问题，包括线程死锁，字符串数组越界等问题
     * 这里使用了 ThreadLocal，进行线程独享，这样就可以避免并发冲突了
    * */
